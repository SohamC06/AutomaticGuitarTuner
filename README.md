# Automatic Guitar Tuner
An automatic guitar tuner that eliminates the need of human intervention to tune a guitar. It is designed to accurately detect the pitch of each string and guide the motor to rotate precisely.

## Demonstration video
Youtube Link for Demo video - https://youtu.be/gi7TasY6R2A

## Components used 
ESP32 Microcontroller
INMP441 Microphone
MG995 Servo Motor(360 degree continuous rotation)
Buck converter
Lithium ion batteries (3 X 3.7V)

## Circuit Diagram
![Circuit Diagram](images/Circuit%20Diagram.png)

## Process Flowchart
![Process Flowchart](images/Process%20Flowchart.png)

## Prototype Image
![Prototype](images/Prototype%20Image.png)
